  ...
  describe '#highlight_all' do
    ...
    context 'given cshtml' do
      ...
      it 'highlights csharp codes' do
        expect(subject.highlight_all!('@foreach(var item in Model) {')).to eq('@<span class="keyword">foreach</span>(<span class="keyword">var</span> item <span class="keyword">in</span> Model) {')
      end

      it 'highlights asp code with csharp model codes' do
        expect(subject.highlight_all!('<li>@item.Name</li>')).to eq('<span class="symbol">&lt;</span><span class="html">li</span><span class="symbol">&gt;</span>@item.Name<span class="symbol">&lt;/</span><span class="html">li</span><span class="symbol">&gt;</span>')
      end

      it 'highlights asp model declaration' do
        expect(subject.highlight_all!('@model List<ForgingAhead.Models.Quest>')).to eq('@<span class="html">model</span> List&lt;ForgingAhead.Models.Quest&gt;')
      end
