...
describe TagHelper do
  ...
  describe "#initialize" do
    ...
    describe 'raises an error' do
      it 'rejects parameter if both :tags and :tag_line parameter provided' do
        expect { TagHelper.new(tags: 1, tag_line: 1)}.to raise_error(AssertionError)
      end

      it 'rejects parameter if neither :tags nor :tag_line parameter provided' do
        expect { TagHelper.new }.to raise_error(AssertionError)
      end

      it 'rejects parameter if tags: is not list of symbols' do
        expect { TagHelper.new(tags: ['Concept']) }.to raise_error(AssertionError)
      end
    end
  ...
  describe '#add' do
    ...
    it 'should include added tag' do
      expect{ subject.add(:FB) }.to change{ subject.include?(:FB)}.from(false).to(true)
    end
  ...
  describe '#index_enum' do
    ...
    describe 'given 2 cards' do
      ...
      it 'removes "EnumU"' do
        expect{ subject.index_enum(cards) }.to change{ subject.include?(:EnumU)}.from(true).to(false)
      end

      it 'adds "EnumU:2"' do
        expect{ subject.index_enum(cards) }.to change{ subject.include?(:'EnumU:2')}.from(false).to(true)
      end
